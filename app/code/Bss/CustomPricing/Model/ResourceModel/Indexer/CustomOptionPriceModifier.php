...
class CustomOptionPriceModifier
{
     ...
     *
     * @param string $idxTblName
     * @param array $changedData
     * @return void
     * @throws \Exception
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
     */
    public function modifyPrice($idxTblName, array $changedData = []) : void
    {
        // no need to run all queries if current products have no custom options
        if (!$this->checkIfCustomOptionsExist($idxTblName)) {
            return;
        }

        $connection = $this->getConnection();
        $finalPriceTable = $idxTblName;
        ...
        $select = $this->getSelectForUpdate($copTable, $changedData["rule_id"]);
     ...
     * @throws \Exception
     */
    private function checkIfCustomOptionsExist($priceTable): bool
    {
    ...
    private function getSelectForOptionsWithMultipleValues(string $sourceTable): Select
    {
        ...
        $select = $connection->select()
            ->from(
                ...
                ['i.product_id', 'i.customer_group_id', 'i.website_id', 'o.option_id']
            );
     ...
     * @return \Magento\Framework\DB\Select
     */
    private function getSelectForUpdate(string $sourceTable, $ruleId): Select
    {
        $connection = $this->resource->getConnection($this->connectionName);
        ...
        $select = $connection->select()->join(
            ...
            ' AND i.website_id = io.website_id AND i.rule_id = ' . $ruleId,
            []
        );
        $select->columns(
            [

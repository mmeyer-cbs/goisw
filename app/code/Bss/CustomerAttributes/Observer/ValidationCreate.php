...
class ValidationCreate implements ObserverInterface
{
        ...
        foreach ($files as $file => $value) {
            ...
            foreach ($attributeCollection as $attribute) {
                if ($file == $attribute->getAttributeCode() && $attribute->getFrontendInput()== 'file') {
                    $this->getRedirectCreate($attribute, $size, $observer, $name);
                }
            }
        }
        return $this;
     ...
     * @throws \Magento\Framework\Exception\LocalizedException
     */
    private function getRedirectCreate($attribute, $size, $observer, $name)
    {
        $validateInputFile = $this->helper->getValueValidateFile($attribute);
        if (isset($validateInputFile['max_file_size']) &&
            $size && $size > $validateInputFile['max_file_size'] * 1024) {

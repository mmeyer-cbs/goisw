...
class History
{
     ...
     * @return bool|mixed
     */
    public function afterCanAddComment($subject, $result)
    {
    ...
    /**
     * Check sales rep and can comment email
     *
     * @param \Magento\Sales\Block\Adminhtml\Order\View\History $subject
     * @param bool $result
     * @return bool|mixed
     */
    public function aftercanSendCommentEmail($subject, $result)
    {
        if ($this->salesData->canSendOrderCommentEmail($subject->getOrder()->getStore()->getId())) {
            if ($subject->getOrder()->canComment() && $this->helper->isEnable() && $this->helper->checkUserIsSalesRep()) {
                $result = true;
            }
        }
